...
class Gemma3Model(nn.Module):
    ...
    def forward_last_token(
        ...
        *,
        ...
        This is an optimized version of the "forward" method that avoid allocating
        memory for all tokens vocabs whil pre-filling the KVCache.
        So the caller can use this to pre-fill the KVCache, it will allocate less
        memory than the forward() method.
        """
        x = self._forward_hidden(in_idx, pos=pos, kv_cache=kv_cache)
        return self._project_logits(x[:, -1:, :])
